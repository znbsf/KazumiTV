...
相同候选随后在旧电视 WebView 66 完成全部 5 线尝试，集表恢复成功，但 5 线均在媒体发现阶段失败，未进入视频播放；日志多次记录第三方 `xcdn.khfed.com` 脚本语法错误。证据为 `age-roads-tv.txt`，应列为旧 WebView 兼容缺口，不能用现代端结果关闭。语法错误是明确观测，但尚不据此断言它是所有线路失败的唯一原因。
