...
状态：未通过。此项使用真实 baimao 搜索、集表、网页解析和媒体，没有注入播放器错误或替换解析结果；网络受限由 Google TV 模拟器控制台产生，不等同于物理电视断网。
...
两轮结束均恢复三个用户数据存储并核对相等，host finally 恢复网络并读回 unlimited / 0 ms。未清除 Cookie、未修改证书策略、未进行整集测试。公开 Preview 4 不包含本轮本地修复。
