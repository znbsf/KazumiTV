...
**当前公开测试版：0.3.3-preview.4。** 基础浏览和内置播放已可试用，仍在迁移与验证阶段；不是上游官方TV版，也不是全功能稳定版。

[下载原生测试版](https://github.com/znbsf/KazumiTV/releases/tag/v0.3.3-preview.4) · [旧Flutter TV版](https://github.com/znbsf/KazumiTV/releases/tag/v2.3.1-tv-legacy.1) · [功能与验证台账](docs/MIGRATION-STATUS.md) · [下一步](docs/ROADMAP.md) · [本轮执行记录](docs/PLAYBACK-MIDDAY-20260922.md)

本轮测试版（**0.3.3-preview.4，code50**）：修复 MXdm 限频页面误报空结果、混合在线/离线历史的原来源入口和详情返回焦点；改进旧WebView验证控件兼容、人工等待恢复和播放器焦点。公开APK不内置弹幕凭证，可在设置自行填写。190项单测及lint通过；真实来源、受控能力和不同构建的验证范围分别记录，不把短时通过当作完整迁移。详见[发行说明](docs/RELEASE-0.3.3-preview.4.md)、[晚间证据](docs/PLAYBACK-EVENING-20260922.md)和[逐源验证表](docs/SOURCE-VALIDATION-20260922.md)。
...
| 弹幕 | 弹弹play只读接入、自动/手动匹配、开关、偏移校准；Preview 4不内置凭证，需在设置填写有效AppId/AppSecret后重新进入播放 |
...
“已实现”不等于所有路径和设备均已验收。当前190项单元测试通过，真机新增系统回收后恢复原集/位置并保持暂停、显式继续播放的证据；搜索、换线及会话组件恢复专项通过。既有真实选集续播、弹幕、过期地址重解析、系统换集等证据按各自构建记录。真实站点、受控测试和未验证项分别列在台账中。
...
| 版本 | 0.3.3-preview.4 | 2.3.1-tv-legacy.1 |
