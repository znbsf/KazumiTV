...
“已实现”不等于所有路径和设备均已验收。本轮已有183项单元测试和多批真机测试：实际第12集选择后自动进入播放、返回选集与续播通过；内置凭证自动匹配7310条弹幕并在画面显示；重解析过期地址、系统上一下一集、HOME后台/前台恢复及组件重建通过。真实站点、受控视频、旧版本整集测试和未验证项在台账中分别记录。
...
- Sorani已有第01集完整23分42秒、自然自动续播第02集的电视记录；仅代表该样本与测试包。待机/网络切换、操作系统杀进程恢复、其他片源长时播放与热稳定尚未全面验收。
